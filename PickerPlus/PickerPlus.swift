...
struct PreviewPickerPlus: View {
    @State private var selectedItem : MusicTabs? = .artists
                ...
                indicatorBuilder: {
                    GeometryReader { geo in
                        Rectangle()
                            ...
                            .cornerRadius(6.0)
                            .padding(1)
                            .frame(width: geo.size.width / CGFloat(MusicTabs.allCases.count))
                            .shadow(color: .black.opacity(0.1), radius: 2, x: 1, y: 1)
                            .animation(.spring().speed(1.5))
                            .offset(x: geo.size.width / CGFloat(MusicTabs.allCases.count) * CGFloat(MusicTabs.allCases.firstIndex(of: selectedItem!)!), y: 0)
                    }.frame(height: 64)
                }
            ) { item in
                HorizontalItem(
